...
function btn_ord_on_mouseup(objInst)
{
    ...
    console.log("현재 슬라이드뷰 인덱스: " + sld.getitemfocus()); // 현재 슬라이드뷰의 인덱스를 로그로 출력

    // 현재 슬라이드뷰의 다음 인덱스 가져오기
    let nNextIdx_LP = sld_LP.getitemfocus() + 1;
    console.log("현재 슬라이드뷰 인덱스: " + sld_LP.getitemfocus()); // 현재 슬라이드뷰의 인덱스를 로그로 출력

    console.log("다음 인덱스: " + nNextIdx); // 다음 인덱스를 로그로 출력
    ...
    console.log("oScrBiz 멤버 확인: ", oScrBiz_LP.getmembers()); // oScrBiz의 멤버 로그 출력
