...
{		
	...
	if(result) {
		...
		if(nFindRow == -1)
		{
			...
			// "fld_Cir" 배열에서 새로 추가된 행(addRow)에 해당하는 인덱스를 사용하여해당 필드에 메뉴 가격(cost)을 설정
...
function fn_setPanelwidth()
{
	...
	// 4. 항목의 너비를 가져와서 리스트 너비를 조정
	let item_width = this.list_Menu.getitemwidth(0);
	
	// 5. 주문 항목 수에 따라 리스트 너비를 다르게 설정
	if(count > 3) {
		this.list_Menu.setwidth(count * item_width); // 항목이 3개보다 많으면, 항목 수에 비례하여 리스트 너비 설정
	}
	else {
		this.list_Menu.setwidth(746);       // 항목이 3개 이하일 경우, 고정 너비인 330으로 설정
	}
}