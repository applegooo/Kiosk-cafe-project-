...
function btn_next_on_mouseup(objInst)
{
	...
	let sld = parentScr.getinstancebyname("SV_Template"); // 부모 화면에서 이름이 "SV_Template"인 인스턴스를 슬라이드뷰로 가져옴
	let sld_LP = parentScr.getinstancebyname("SV_Template_LP");
	
	let nNextIdx = sld.getitemfocus() + 2;	    // 현재 슬라이드뷰에서 포커스된 아이템의 다음 인덱스를 가져옴
	
	// 슬라이드뷰 포커스를 설정하고, 다음 화면으로 이동
	
	sld.setitemfocus(nNextIdx);                 
    //sld.setfocus();
	sld.movenext();
	sld_LP.movenext();
}
...
function btn_no_on_mouseup(objInst)
{
	...
	let oScrBiz = SYSUtil.fn_getBizScreen(this, nNextIdx_2);    // nNextIdx_2 해당하는 비즈니스 화면(BizScreen) 인스턴스를 가져옴
	let objTime  = oScrBiz.getmembers().timeLeft;   // 가져온 BizScreen에서 "timeLeft"이라는 멤버 객체를 objTime에 할당
	...
	let obj_10 = objTime.getenable();	// 현재 화면에서 "timeLeft" 타이머 활성화 상태를 time 에 저장 	
